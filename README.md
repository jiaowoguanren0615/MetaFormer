<h1 align='center'>MetaFormer Baselines for Vision (TPAMI 2024)</h1>
...
├── models: MetaFormer Model
    ├── build_models.py: Construct MetaFormer models(caformer, poolformer, convformer, identityformer, randfomrer)
    ├── model_configs.py: default configs for entil models
├── util:
    ├── engine.py: Function code for a training/validation process
    ├── losses.py: Knowledge distillation loss, combined with teacher model (if any)
    ├── optimizer.py: Define AdamAtan2/MARS optimizer
...
## Use AdamAtan2 & MARS Optimizer (in util/optimizer.py)
You can use anther optimizer AdamAtan2 & MARS, just need to change the optimizer in ___train_gpu.py___, for this training sample, can achieve better results
```
...
```
### Transfer Learning:
Step 1: Download the [pretrained-weights](https://github.com/sail-sg/metaformer/blob/main/README.md#metaformer-baselines)  
...
```bash
python onnx_export.py --model=caformer_m36 --output=./caformer_m36.onnx --checkpoint=./output/caformer_m36_best_checkpoint.pth
```

### step2: ONNX optimise
```bash
python onnx_optimise.py --model=caformer_m36 --output=./caformer_m36_optim.onnx'
```

### step3: ONNX validate (modify the param of ___data_root___ and ___onnx-input___)  
```bash
python onnx_validate.py --data_root=/mnt/d/flower_data --onnx-input=./caformer_m36_optim.onnx
```
